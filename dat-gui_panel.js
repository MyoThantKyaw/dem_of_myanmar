Q3D.gui = {
  ...
  init: function (setupDefaultItems) {
    ...
    if (setupDefaultItems === undefined || setupDefaultItems == true) {
      ...
      this.customPlaneFolder = this.gui.addFolder('Level မျက်နှာပြင်');
  ...
  initCustomPlaneFolder: function (zMin, zMax) {
    ...
    var addPlane = function (color) {
      ...
      var geometry = new THREE.PlaneBufferGeometry(p.width,p.height, 1, 1),
          material = new THREE.MeshLambertMaterial({color: color, transparent: true, opacity : .5});
    ...
    // Plane color
    this.customPlaneFolder.addColor(parameters.cp, 'c').name('အရောင်').onChange(function (value) {
    ...
    // Plane altitude
    this.customPlaneFolder.add(parameters.cp, 'd').min(zMin).max(zMax).name('အမြင့် (မီတာ)').onChange(function (value) {
      if (customPlane === undefined) addPlane(parameters.cp.c);
    ...
    var guiFrame = document.getElementsByClassName("main");
    guiFrame[0].style.marginRight = "0px";
  },
